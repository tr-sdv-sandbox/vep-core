...
// Convert DDS vss_types_Value to kuksa vss::types::Value
vss::types::Value dds_to_kuksa_value(const vss_types_Value& dds_value) {
...
// Convert kuksa Value to DDS vss_types_Value
void kuksa_to_dds_value(const vss::types::Value& value, vss_types_Value& dds_value) {
    // Initialize to empty
    dds_value = {};
...
void KuksaDdsBridge::dds_poll_loop() {
    ...
    while (running_) {
        ...
        try {
            dds_signals_reader_->take_each<vss_Signal>(
        ...
        try {
            dds_actuator_actual_reader_->take_each<vss_Signal>(
                [this](const vss_Signal& signal) {
...
void KuksaDdsBridge::on_dds_signal(const vss_Signal& signal) {
...
void KuksaDdsBridge::on_dds_actuator_actual(const vss_Signal& signal) {
    std::string path = signal.path ? signal.path : "";
    if (path.empty()) {
...
void KuksaDdsBridge::publish_to_kuksa(const std::string& path, const vss_Signal& signal) {
